...
describe('useIsClient()', () => {
	test('should be false when rendering on the server', (): void => {
		const { result } = renderHookSsr(() => useIsClient());
		expect(result.current).toBe(false);
	});

	test('should be true when after hydration', (): void => {
		const { result, hydrate } = renderHookSsr(() => useIsClient());
		hydrate();
		expect(result.current).toBe(true);
	});

	test('should be true when rendering on the client', (): void => {
